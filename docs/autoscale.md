...
生產環境應部署經審查的 release wheel，不應直接從 `main`、Git URL 或可變動的工作目錄安裝。完整步驟、虛擬環境路徑、BCC 系統套件、systemd unit、MQTT TLS／密碼檔、檔案權限與驗收清單請參考 [Production deployment guide](../autoscale/docs/PRODUCTION_DEPLOYMENT.md)。可從 [config/adjust.env.example](../autoscale/config/adjust.env.example) 與 [config/machines.yaml.example](../autoscale/config/machines.yaml.example) 建立站點設定。
